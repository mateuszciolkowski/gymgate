...
interface DataContextType {
  ...
  // Akcje - Plans
  createPlan: (data: { name: string; shortName?: string; exerciseIds: string[]; isPublic: boolean }) => Promise<WorkoutPlan>;
  updatePlan: (id: string, data: { name?: string; shortName?: string; exerciseIds?: string[]; isPublic?: boolean }) => Promise<void>;
...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const createPlan = useCallback(
    async (data: { name: string; shortName?: string; exerciseIds: string[]; isPublic: boolean }): Promise<WorkoutPlan> => {
  ...
  const updatePlan = useCallback(
    async (id: string, data: { name?: string; shortName?: string; exerciseIds?: string[]; isPublic?: boolean }) => {
      if (!navigator.onLine) throw new Error("Brak połączenia z serwerem — spróbuj ponownie gdy będziesz online.");
